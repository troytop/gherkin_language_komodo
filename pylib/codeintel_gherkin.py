...
################################################################################
lang = "Gherkin"
log = logging.getLogger("codeintel.gherkin")
#log.setLevel(logging.DEBUG)

# These keywords are copied from "gherkin-mainlex.udl"
...
################################################################################
class GherkinLexer(UDLLexer):
        lang = lang
################################################################################
class GherkinLangIntel(LangIntel):
    lang = lang
...
class GherkinBuffer(UDLBuffer):
    ...
    cpln_stop_chars = " ()*-=+<>{}[]^&|;:'\",.?~`!@#%\\/"
################################################################################
class GherkinCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_gherkin
        return cile_gherkin.scan_buf(buf)
################################################################################
def register(mgr):
    """Register language support with the Manager."""
    mgr.set_lang_info(
        lang,
        silvercity_lexer=GherkinLexer(),
        buf_class=GherkinBuffer,
        langintel_class=GherkinLangIntel,
        import_handler_class=None,
        cile_driver_class=GherkinCILEDriver,
