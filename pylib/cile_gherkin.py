...
"""A Code Intelligence Language Engine for the Gherkin language.
...
Module Usage:
    from cile_gherkin import scan
    mtime = os.stat("bar.gherkin")[stat.ST_MTIME]
    content = open("bar.gherkin", "r").read()
    scan(content, "bar.gherkin", mtime=mtime)
"""
...
class GherkinCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.gherkin")
...
def scan_buf(buf, mtime=None, lang="Gherkin"):
    """Scan the given GherkinBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {GherkinBuffer} is the Gherkin buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Gherkin CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Gherkin content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Gherkin <example-Gherkin-file>
    ...
    # Dev Note:
    # This is where you process the Gherkin content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Gherkin-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Gherkin content, you may
    #   want to use it.
